...
2. In this folder run:
   ...
   git init
   git add .
   git commit -m "UENR payment portal"
   git branch -M main
...
3. Vercel detects `vercel.json` automatically — just click Deploy (no settings needed).
4. On every deploy, Vercel automatically runs migrations and collects
   static files (see the `buildCommand` in vercel.json):
   `python3 manage.py migrate --noinput && python3 manage.py collectstatic --noinput`

## Hosted Postgres (persistent data on Vercel)

SQLite only works locally — on Vercel it resets every deploy. To keep data
permanently, use hosted Postgres:

**Option A — Vercel Postgres**
1. In your Vercel project: Storage → Create Database → Postgres.
2. Vercel sets the connection variables; add `DATABASE_URL` in
   Project Settings → Environment Variables (copy the connection string).

**Option B — Neon (free tier)**
1. Sign up at neon.tech, create a project, and copy the connection string
   (it looks like `postgresql://user:pass@host/dbname?sslmode=require`).
2. In Vercel: Project Settings → Environment Variables → add
   `DATABASE_URL` with that value.

Then redeploy. When `DATABASE_URL` is set, Django uses Postgres and SQLite
is ignored; without it, local development still uses SQLite, so nothing
changes on your own computer.
